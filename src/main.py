...
def get_or_create_cart(cur, customer_id):
    cur.execute(
...
def get_cart_items(cur, customer_id):
    cur.execute("""
...
def show_cart_items(customer_id):
    db = conn(); cur = db.cursor()
    rows = get_cart_items(cur, customer_id)
    header('Cart')
    if not rows:
        print('  Cart is empty.')
...
def add_product_to_cart(customer_id, barcode, quantity):
    ...
    cart_id = get_or_create_cart(cur, customer_id)
    cur.execute(
...
def buy_cart(customer_id):
    db = conn(); cur = db.cursor()
    items = get_cart_items(cur, customer_id)
    header('Buy Cart')
    if not items:
        print('  Cart is empty.')
        db.close(); pause(); return
